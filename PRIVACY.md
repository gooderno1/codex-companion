...
- 额度估算索引：`quota-estimation-*.sqlite` 及 WAL / SHM 文件，保存文件路径 / 签名、会话 ID、事件时间、模型、原始 Token 计数和结构化额度观测；不保存任务名称、项目路径、正文、凭据或派生成本。数据仅用于本地按月估值和窗口证据计算，价格切换不改写原始记录。
- 额度估算页面偏好：本地存储保存视图、日期范围、价格快照及排序；不上传筛选、用量、模型或额度信息。页面不新增联网请求，价格随应用 Release 交付。
