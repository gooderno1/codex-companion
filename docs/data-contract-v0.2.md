...
- 新增 `quota:estimation` / `getQuotaEstimation`，请求为 `startAt: ISO | null, endAt: ISO, catalogVersion?: string, priceMode?: snapshot | historical, force?: boolean`。范围左闭右开，终点截到当前时间；未知目录或非法范围拒绝。默认当前目录，历史模式不以 verifiedAt 代替 effectiveFrom。
- 返回 `months / windows / summary / catalogs / coverage / exclusions / performance`；不依赖仪表盘或 Git，原始行不跨 IPC，只返回统计和样本证据。UI 可直接进入 `#/quota-estimation`，未取得主快照仍可加载。
- 后台 Worker 串行维护 `quota-estimation-*.sqlite`，原始格式版本 1 与价格目录版本分离。保存现有解析器的 Token 增量与 quotaObservations，额度计数按窗口时长识别；不新增解析 / reset 规则。只在首次或文件签名变化时解析；60 秒内普通查询复用索引，手动重读强制检查签名。退出和数据源切换时关闭线程并拒绝过期请求。
- 查询使用时间索引，只加载范围及最多一周的边界上下文；范围内月表按本地自然月求和，空月金额 null。`costUsd=null` 表示全未定价或无记录，`unpricedEvents / unpricedTokens` 区分已知部分，Token 定价比例不是账户 / 成本完整度。
- 固定快照可选当前和内置历史目录；采用标准短上下文费率，未还原 Fast、缓存写入、长上下文、工具或地区。历史模式只接受 `effectiveTimeBasis=official` 且覆盖事件时间的目录；当前缺少此类证据时显示未定价，不伪造当时价格。
- 相同会话 ID / 时间 / 模型 / Token 计数组合去重；不承诺可以识别所有继承会话或其他设备的重复。主池窗口只使用 codex 和单独标明的 legacy；Spark 用量不参与主池容量成本与未知比例，月表仍计入其原始 Token。
- 周窗口 duration=10080；重置截止最多按锚点 120 秒合并，5 分钟桶取中位数。共享核心 `analyzeQuotaObservations` 的 confirmed 边界用于切片和排除迟到旧窗口；不把 reset 检测数当作银行额度兑换数。同边界下降超过 3 点、套餐 / 池变化和超过 48 小时采样中断也切片，未确认回落只标明原因未知。
- 样本至少增长 10 个百分点、最长 48 小时，成本和 Token 均大于 0、未知主池 Token ≤ 2%；排除跨选择范围 / 确认边界 / 套餐变化和 99% / 100% 截顶。满额等价值为 `100 × sum(ΔC) / sum(Δp)`，不是官方预算；如 $90 / 30% 得 $300。
- 样本至少 3 段、累计至少 30 点，且 `(P90-P10)/估值 ≤ 0.5` 才标为中等证据；其余有值样本为探索性，无合格样本不估值。0.5 为初版产品质量门槛，不是官方规则；经验分位和 ±2 分钟对齐范围不称为置信区间。
- 同池 / 同 plan、间隔不超过一周的中等证据片段，连续最后 3 个样本连同 ±2 分钟敏感性范围都高于前段估值 20% 或低于 20% 时提示换算关系变化；不称为官方调价或缩减，不发额度提醒。
- 图形仅绘制实际样本期，不填补观测缺口；详情列出每段 ΔC / Δp、经验离散区间、排除原因、原始 plan 字段及价格来源。主余量、通知和现有账本数据契约保持不变。
...
- 价格目录与历史边界详见 [2026-09-23 定价核查](./model-pricing-2026-09-23.md)。随包保留 2026-09-17 / 2026-09-19 旧价格快照；当前界面按 2026-09-23 标准价重估全部可见历史，不声称按事件时点的实际费率计账。
...
- 模型匹配接受已登记名称、已登记基础型号的日期后缀，以及官网明确的 gpt-5.6 / Daybreak Blue / Red 别名；未知型号保持未定价。GPT-6 Astra 标准 credits 为 250 / 25 / 1250 每百万输入 / 缓存读取 / 输出，不通过美元反推。
- 会话缓存版本为 4，SQLite 索引 user_version 为 104；即使源文件未变化，旧成本也重新计算。仪表盘 pricingCatalogVersion 必须匹配 2026-09-23，启动和采集失败均不得复用旧价格快照；原始 Token 保持不变。
- 周套餐价值折算公式：
...
- 不上传原始 session
- 不读取 GitHub 远端 API
- 未定价模型的估值函数返回 null；事件 apiPricingStatus / creditPricingStatus 标为 unpriced。兼容数值聚合字段只累加已知部分，内部加数 0 不表示免费；模型详情继续展示未定价。
