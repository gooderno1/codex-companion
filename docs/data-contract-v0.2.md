...
- 价格目录与历史边界详见 [2026-09-19 定价核查](./model-pricing-2026-09-19.md)。随包保留 2026-09-17 旧价格快照；当前界面按 2026-09-19 标准价重估全部可见历史，不声称按事件时点的实际费率计账。
...
- 会话缓存版本为 3，SQLite 索引 user_version 为 103；即使源文件未变化，旧成本也重新计算。仪表盘 pricingCatalogVersion 必须匹配 2026-09-19，启动和采集失败均不得复用旧价格快照；原始 Token 保持不变。
- 周套餐价值折算公式：
