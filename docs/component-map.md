...
| 5H 额度卡 | `QuotaWindowCard` | 可复用 | `windowData`、`period`、`models`、`period.quotaEvidence` | 按时长从 primary / secondary 中选择 `300` 分钟窗口；不存在时 `limitWindows[0]` 标记未观测并展示契约未提供说明，不复用 7 天窗口 |
| 周额度卡 | `QuotaWindowCard` | 可复用 | `windowData`、`period`、`models`、`period.quotaEvidence` | 按时长从 primary / secondary 中选择 `10080` 分钟窗口；新版仅 primary 时仍正常生成周期、余量与通知；reset 检测来自 `@lifeinhand/codex-usage-core@0.1.0-dev.11` |
...
- 应用更新由主进程 `UpdateService` 维护唯一状态；启动检查延迟约 `15s`，周期检查间隔 `6h`，不复用 5 分钟仪表板刷新；临时未签名 Windows NSIS 构建允许自动下载和用户确认安装，但 `canInstallOnQuit=false`，renderer 必须展示未签名与 SmartScreen 风险。确认后由 `windowsUpdateInstaller` 校验缓存路径和版本文件名，通过当前用户的临时 Windows 计划任务启动 PowerShell helper，观察到 helper 状态后退出，helper 安装结束时自删除任务。
...
- 系统通知只在 live 快照生成后检查：赠送重置按预计过期前 `7d / 3d / 1d / 12h / 1h` 里程碑提醒，首次观测前已有 credit 无法反推时只提醒一次；5H / 周额度剩余量低于 `20%` 或 `10%` 时按当前额度周期提醒一次；`quiet` 只进入应用内通知页，`off` 不生成新提醒。
