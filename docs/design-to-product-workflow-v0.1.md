...
当用户基于真实截图指出“整体方向正确，但颜色、基线、单行展示、重复文案等细节仍偏离设计图”时，不允许直接临时改 CSS。必须先做一轮轻量文档闭环：

1. 在页面 `ui-contract` 中补充具体差异规则，例如品牌图标与标题顶线、时间视角单行、额度卡标题只写一次。
2. 如果涉及颜色、文字色阶、边框或阴影，先更新 `docs/design-tokens-v0.1.md` 与 `src/renderer/design-tokens.ts`。
3. 同步更新 `scripts/verify-design-tokens.mjs` 的核心 token 期望，确保后续不会意外回退。
4. 如果涉及图标，必须进入统一图标模块或资产目录；页面组件只引用图标资产，不在各页面复制 SVG。
5. 如果涉及辅助说明文案，必须确认它是否会额外占用一行高度；可放入标题行或状态证据行时，不应新增独立说明行。
6. 再改组件 CSS 或数据文案。
7. 重新生成 `1360 x 900` 与 `1080 x 720` 真实 Electron 截图，并把差异结论写回分区设计审计。

这条规则用于减少后续页面返工：视觉细节必须进入可执行约束，否则同一问题会在账本页、仓库页和设置页重复出现。
...
- 验证记录
- 设计对照结论
- 页面级验收命令结果，例如总览页的 `npm run verify:overview`
