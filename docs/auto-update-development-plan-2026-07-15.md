...
- `v0.5.2`（2026-09-01）发布总览变化百分比 / 数值切换和零分母具体百分比；稳定版更新源、自动下载和用户确认安装链路不变。
- `v0.5.1`（2026-09-01）发布总览上一周期同期对比修复；稳定版更新源、自动下载和用户确认安装链路不变。
- `v0.5.0`（2026-08-31）发布可配置的 Windows 开机自启；稳定版更新源、自动下载和用户确认安装链路不变。
- `v0.4.6`（2026-07-31）发布 Codex 官方 Usage 当前额度采集正式版；继续沿用固定 stable Release、自动下载和用户确认安装链路。
- `v0.4.6-dev.1`（2026-07-31）升级 `codex-usage-core v0.2.0-dev.1`；当前额度改为官方 Usage 优先、本地 session 回退，官方缺少窗口时保持未观测。
- `v0.4.5`（2026-07-28）发布自动下载交互补强与更新界面精简正式版。
- `v0.4.5-dev.1`（2026-07-28）复核并补强自动检查、自动下载链路：正式 Windows NSIS 安装版在开关开启时启动约 `15s` 检查、每 `6h` 周期检查；发现未忽略的新版本后自动下载，若已发现更新时才打开自动下载，也立即开始下载。更新卡片、全局提示和详情弹层移除重复解释小字，保留版本、状态、进度、错误和操作。
...
- 更新通道：首版固定显示“稳定版”。
- 自动检查：默认开启，可关闭；关闭后仍保留“检查更新”，页面只呈现开关、状态和操作，不重复解释调度间隔。
- 自动下载：Windows NSIS 安装版默认开启，可关闭；页面只呈现开关、下载进度和操作，未签名边界保留在公开文档、Release 说明和系统安全提示中。
