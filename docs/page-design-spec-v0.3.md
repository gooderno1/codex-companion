# Codex Companion 页面设计规格（v0.3.29 草案）

- 创建时间：2026-06-03
- 对应阶段：页面视觉重设前的设计确认
- 当前目标：先确认页面内容、展示方式、尺寸和位置，再进入实际页面开发
- 涉及页面：总览页、Codex 账本页、代码仓库页、设置页
- 暂缓范围：桌面挂件细化开发。挂件本轮仅改为默认关闭，后续单独设计。

## 0. 修订记录

- `v0.7.0-dev.1`（2026-09-21 实施补充）：新增与 Codex 账本并列的“额度估算”。顶部统一范围 / 价格筛选，内部切换按月已用成本与按窗口估值；月度详情展示模型及每日用量，窗口详情展示公式、样本和剔除原因。表格先全量排序再分页，Token 使用全应用统一单位，图表仅绘制有效样本区间。完整设计与边界见 [额度估算计划](quota-capacity-pricing-research-plan-2026-09-17.md)。
...
- `v0.3.27`：按最新反馈把第二页 `Token 走势拆解` 放大态从右上浮层改为上下结构：默认隐藏上方明细，点击曲线点位、横轴日期或 `明细` 后，上方展开当前粒度表，下方保留曲线图；非放大态仅优化右侧明细栏的日期与指标排版。产品开发版本同步提升到 `v0.3.0-dev.6`。
- `v0.3.28`：按确认后的二次设计落地第二页 `Token 走势拆解` 明细排版。放大态上方明细带的右侧表格固定为 `曲线 / 数值 / 占比` 三列；非放大态顶部摘要去掉 `缓存输入占比`，只把 `会话 / API 等价成本` 放到日期右侧。产品开发版本同步提升到 `v0.3.0-dev.7`。
- `v0.3.29`：继续修正第二页 `Token 走势拆解` 放大态明细带视觉。上方明细带改为更接近设计图的稳定信息面板，日期、摘要卡和三列表格在同一视觉高度内对齐，表格字号和行距放大。产品开发版本同步提升到 `v0.3.0-dev.8`。
...
- 顶部全局状态表达本次数据源是否成功观测；最近没有新 token 事件只通过 `最新观测` 时间说明，不单独触发 `数据过期`。
...
设置页首轮承载轻量运行配置和刷新追溯，不做完整偏好中心。当前至少覆盖：
...
- 顶栏下方允许一条临时轻量刷新反馈条，展示 `增量采集 / 刷新完成 / 刷新失败` 等状态；该反馈条服务所有页面，完成或失败约 `5s` 后消失，不作为常驻状态栏。
...
- 次级说明使用增减百分比或状态说明：
  ...
  - `较上月同期 +15.2%`
  - `较昨日同期 -3.1%`
- 变化百分比统一使用上一周期同期作为分母，不使用完整上一日、上一周或上一月。
...
- 当页级视角为 `计费时间` 时，项目概览提供 `5H / 周额度 / 计费月` 二级切换；计费月起点来自设置页 `billingMonthStartDay`。
...
- 项目概览在计费时间下必须包含 `5H / 周额度 / 计费月`，不再只保留前两项。
...
- `日 / 周 / 月` 原设计下沉到 `Token 走势拆解` 模块头部，只作为该分析模块的局部状态；当前实现开放三种粒度。
...
- 左侧主卡：`Token 走势拆解`
  - 占 `68%` 宽度，高度 `356px`
  - `日 / 周 / 月` 视角切换放在模块头部，而不是页面顶部工具栏；三种粒度均可用
  - `日` 为小时粒度，`周 / 月` 为日期粒度
  ...
  - 图形规则：
    ...
    - 当前粒度表头部不重复展示日期范围，非放大态只把 `会话 / API 等价成本` 放到日期右侧
...
- 代码改动卡次级说明使用改动趋势：
  - `较昨日同期 -3.1%`
- 顶部工具栏提供“变化显示 百分比 / 数值”切换；百分比为默认并跨重启保留，数值模式只替换四卡次级说明，不改变主数字。
- 百分比的上一周期同期为 `0` 时，仅把分母按 `1` 计算；超大结果紧凑为 `万% / 亿%`，完整结果放入悬停提示。数值模式使用真实差值并追加 `tok / 行`。
...
- `Token 走势拆解` 主图升级为 `日 / 周 / 月` 多曲线图；`总 Token / 输入总量 / 原始输入 / 缓存输入 / 输出 / 推理 Token` 通过图例显隐控制。
- 点击某个曲线点位后，右侧当前粒度表展示该小时或日期的曲线数值、占比、会话数、API 等价成本和缓存输入占比。
...
- 开发版本提升到 `v0.3.0-dev.1`，表示账本页趋势模块进入多粒度、多曲线交互阶段。
- `Token 走势拆解` 恢复 `日 / 周 / 月` 三种粒度，分别使用小时或日期粒度数据桶。
- 主图改为 SVG 多曲线图，支持 `总 Token / 输入总量 / 原始输入 / 缓存输入 / 输出 / 推理 Token`。
- 图例按钮支持显示或隐藏单条曲线，且至少保留一条曲线可见。
- 点击点位后右侧显示当前小时或日期粒度表，表格展示曲线数值和占比，并补充会话、API 等价成本和缓存输入占比。

### 12.5 v0.3.24 当前修正说明

- 开发版本提升到 `v0.3.0-dev.3`，表示账本页趋势模块进入平滑曲线和放大查看阶段。
- `Token 走势拆解` 主图使用受约束的平滑 SVG path，点位圆点、tooltip 和命中区仍对应真实桶数据。
- `Token 走势拆解` 卡片头部新增 `放大查看` 图标按钮。
- 放大层覆盖主内容工作区，左侧展示大尺寸趋势图，右侧展示当前粒度表。
- 放大层与原卡片共享 `日 / 周 / 月`、曲线显隐和选中点位状态，关闭后不重置用户选择。

### 12.6 v0.3.25 当前修正说明

- 开发版本提升到 `v0.3.0-dev.7`。
- 全局 `sourceStatus` 不再因为最新 token 事件超过 `45` 分钟而自动显示 `数据过期`。
...
- 开发版本提升到 `v0.3.0-dev.6`。
- 放大态 `Token 走势拆解` 使用上下结构：上方当前粒度明细、下方曲线图。
- 放大态默认隐藏上方明细；点击曲线点位、横轴日期或 `显示明细` 后展开。
- 非放大态仍保留右侧明细栏，只优化日期与 `会话 / API 等价成本 / 缓存输入占比` 的排版，避免一行内拥挤。

### 12.9 v0.3.28 当前修正说明

- 开发版本提升到 `v0.3.0-dev.7`。
- 放大态上方明细带的右侧表格必须显示 `曲线 / 数值 / 占比` 三列，且六条曲线行完整可见。
- 非放大态顶部摘要移除 `缓存输入占比`，只保留 `会话 / API 等价成本` 并放在日期右侧。
- `缓存输入` 仍作为明细表曲线行展示，统计口径不变。

### 12.10 v0.3.29 当前修正说明

- 开发版本提升到 `v0.3.0-dev.8`。
- 放大态上方明细带不再表现为压扁的表格条，而是稳定的信息面板。
- 左侧日期和 `会话 / API 等价成本 / 缓存输入占比` 三项摘要需要使用等高浅底卡片，垂直居中对齐。
- 右侧明细表保持 `曲线 / 数值 / 占比` 三列，字号、行高和单元格内边距必须适合放大态阅读。
- 非放大态趋势明细布局与数据口径不在本次修正范围内。
