...
- 包含 v0.7.0-dev.1 的全部功能及 v0.7.0-dev.2 的发布准备；未排除开发版本。
- 合入 PR #1；共享核心固定远程 tag @lifeinhand/codex-usage-core@0.2.0-dev.4，模型价格继续使用 2026-09-19 快照。

### 主要变更与修复

- 新增一级“额度估算”，提供月度标准 API 等价成本、模型与每日用量详情，以及周窗口满额等价值和计算依据。
- 支持近 6 / 12 个月、全部历史、自定义日期、价格快照切换、表头升降序、分页与页面状态恢复；Token 与总览统一换算为万 / 亿。
- 周窗口按实际比例增量分段估算，展示样本门槛、经验 P10–P90、时间对齐敏感性及剔除原因；不把估值当作官方套餐预算。
- 后台线程与独立 SQLite 原始索引支持增量更新、归档去重和跨重启复用；价格变化只重算估值。
- 共享核心复用历史回看中的时间解析，固定 55,866 条真实观测的完整结果与旧版一致；官方当前余量、通知及挂件口径保持不变。
...
- 升级后从左侧进入“额度估算”；首次建立历史索引需要额外时间，期间可以切换其他页面。无需删除 Codex 数据或清理原有缓存。
- 所有金额是本机可见用量的标准短上下文等价成本，不是订阅实付；账户、其他设备、Fast、工具等覆盖可能不完整。
- 当前缺少可靠历史生效价格，“按当时价格”会显示未定价；使用“固定快照重估”可比较各月成本。
- 沿用未签名发布方式；应用内点击“重启并安装”才会安装，普通退出不安装。
...
- 包含 `v0.6.2-dev.1`、`v0.6.2-dev.2`；未排除开发版本。
- 共享核心继续固定 `@lifeinhand/codex-usage-core@0.2.0-dev.3`。

### 主要变更与修复

- 会话列表、详情和峰值说明优先显示 Codex 自动生成或用户修改的名称，保留 ID 供定位；支持名称 / ID 搜索，无名称时回退 ID。
- 名称读取优先使用 `threads.name`，其次使用 `session_index.jsonl.thread_name`；不把任务正文当作名称，不改写 Codex 数据库。
- 详情 Token 与总览统一：小于 10,000 显示整数，10,000 起显示万，100,000,000 起显示亿；悬停查看完整数值。会话数、事件数和金额保持原单位。
- 项目 / 会话详情、模型构成、每日用量和账本会话表支持点击表头升序 / 降序，箭头与高亮显示当前方向。
- 排序比较原始数值；先对全部筛选结果排序再分页，账本先排序已加载会话再取前 8 条，避免只排序当前页。
- 排序保留筛选和选中对象，仅在界面执行，不触发额外数据库、会话解析或 Git 查询。

### 验证结果

- 两个开发版本完整构建与详情 / 名称 / Token 单位 / 排序 / 账本回归通过；开发提交 `9d63815` 的 CI run `35223577118` 成功。
- 真实 Electron 验证 31 个项目、494 个会话的跨页排序、筛选及选中保留、子表和账本排序；整个排序测试仅首次加载触发 1 次详情 IPC。
- 真实界面检查名称搜索、无名回退、完整 ID、Token 精确值提示和 1360 / 1080 窗口布局。
- 正式提交 `2466f87` 的 CI run `35224191078` 与 Windows Package run `35224195497` 成功；正式版完整构建、详情 / 名称 / 排序、更新器和签名政策检查通过。

### Release 资产校验

- 四项正式资产来自同一次 tag 构建 `35224195497`。上传产生两份同标签草稿，已将同源 blockmap 补入完整草稿，并清理本次重复草稿。
- 安装包产品名 / 版本为 `Codex Companion / 0.6.2`，Authenticode 为 `NotSigned`。
- latest.yml 的版本、路径、大小与安装包 SHA512 一致；四项资产的 GitHub digest、工作流产物和匿名下载 SHA256 全部一致。
- `Codex.Companion.Setup.0.6.2.exe`：`102986627` bytes；SHA256 `21d6aae083677d70feea1a341a8acccd16a19c03d2e862dadb16941461588a91`。
- `Codex.Companion.Setup.0.6.2.exe.blockmap`：`108556` bytes；SHA256 `7444ffe2e6b83616b682901a3bf9fb7fda1419960e8e0312b0a45f138049f4dc`。
- `latest.yml`：`359` bytes；SHA256 `be157e2aa452c04ddefabf54569eb1deb0ffb57f997834728bbd360fb8088b59`。
- `SHA256SUMS.txt`：`99` bytes；SHA256 `b45b29ae3d9b5c90c31252c5b7acf9be1dee7805a68f8a1f8881c479b81551df`。
- `v0.6.2` 已公开为 stable Latest；Release 页面和稳定更新入口验证通过。匿名下载首次连接重置，使用系统 curl 重试后全部通过。

### 升级注意事项

- 可从 `v0.6.1` 或 `v0.6.2-dev.*` 升级，保留本机设置与历史索引，无需删除 Codex 数据或清理缓存。
- 升级后刷新数据可读取名称；旧记录没有名称元数据时继续显示 ID。
- API 等价成本仍为标准单价估算，不是账单实付；本次不改变 Token、余量或 reset 统计口径。
- 安装包沿用既有未签名发布方式；应用下载完成后点击「重启并安装」，普通退出不安装。

## [2026-09-17] v0.6.1 release: 修正 Codex 项目归因并加速历史详情查询

### Release 范围

- 包含 `v0.6.1-dev.1`；未排除开发版本。
- 共享核心继续固定 `@lifeinhand/codex-usage-core@0.2.0-dev.3`，本次不修改 Token / reset 算法。

### 主要变更与修复

- 项目概览、会话归因和详情统一对应 Codex 保存的项目，Git 为可选关联；无 Git 项目也可完整查看会话用量。
- 合并 Codex 新旧项目 ID，识别显式归属、工作树、嵌套根目录与项目改名；歧义、无项目和已删除项目的会话保留独立分组，不按 Git 仓库猜测。
- 新增本地 SQLite 持久化索引，仅解析新增或变化文件；时间范围查询命中索引，关闭应用后也可复用。
- 修复归档移动、截断和删除文件后的索引更新；升级时使旧版仓库项目分组缓存失效。
- Git 活动改为选中项目后点击「读取 Git 活动」单独查询，不再阻塞项目和会话列表；无 Git 的代码指标显示「—」。

### 验证结果

- 开发提交 `1c99b20` 的 GitHub CI run `35211450617` 成功；完整构建及项目归属、SQLite 索引、用量、同期、账本回归通过。
- 正式提交 `a55846d` 的 CI run `35219473329` 和 Windows Package run `35219477013` 成功；本地正式版构建、详情、用量、更新器和签名政策检查通过。
- 真实 Electron 验证双入口、非 Git 项目、按需 Git、旧日期、分页搜索、窄窗口、关闭及焦点恢复。
- 本机 526 个文件的历史查询：首次建立索引约 25.2 秒；近 7 天约 0.22 秒，全历史约 0.82 秒，重建服务后约 0.84 秒。结果随历史量和磁盘性能变化。
- 索引样例：累计 100 Token 的文件追加到 180 后总量仍为 180；归档不重复计数，截断和删除移除旧事件；查询计划命中事件时间索引。

### Release 资产校验

- 四项正式资产来自同一次 tag 构建 `35219477013`；上传时生成两份同标签草稿，已将缺失资产补入同一草稿，未混用本机构建产物。
- 安装包 `Codex.Companion.Setup.0.6.1.exe`：`102984092` bytes，SHA256 `872F0C6B695703D1C8883741F557DC36CE99E4A864039867B1081687E50F3EDF`；与 GitHub digest 和 SHA256 清单一致。产品名 / 版本为 `Codex Companion / 0.6.1`，Authenticode 为 `NotSigned`。
- blockmap：`108515` bytes，SHA256 `0346BA743D296E02DE8F3B6CC7788EDA1FE0D937F96317908A3C2CFCEE139586`。
- latest.yml：`359` bytes，SHA256 `663F147AB94A52A50879DD9195A1E173B724DEDE8F6C32F28614FD74583DC965`；版本、路径、大小、SHA512 与安装包一致。
- SHA256SUMS.txt：`99` bytes，SHA256 `766E1A5897A23D3F93117A39C522E4B30E59BFB98D0F88FF978E635DF9AA0E74`。
- `v0.6.1` 已公开发布为 stable Latest；四项资产匿名重新下载后的哈希与工作流产物全部一致，Release 页面可访问，稳定更新入口 latest.yml 与 tag 产物一致。

### 升级注意事项

- `0.6.0` 和 `0.6.1-dev.1` 可升级到 `0.6.1`；保留本机设置，不需要删除 Codex 数据或手工清理缓存。
- 第一次打开历史详情仍需要建立索引，请等待完成；后续查询和重启复用索引。常规仪表盘 Git 刷新保留既有路径，上述耗时只针对详情查询。
- 索引仅保存在 Companion 用户数据目录，不改写 Codex 数据库，不保存对话正文。
- 安装包仍未签名；下载完成后点击「重启并安装」，普通退出不安装。

## [2026-09-17] v0.6.0 release: 项目与会话历史详情、当前余量和 GPT-6 成本修复

### Release 范围

- 包含 `v0.5.3-dev.1`、`v0.5.3-dev.2`、`v0.6.0-dev.1`；未排除开发版本。
- 三个开发版本合并进入 `v0.6.0`，不单独发布 `v0.5.3`。
- 共享核心固定为 `@lifeinhand/codex-usage-core@0.2.0-dev.3`。

### 主要变更

- 项目概览和会话归因新增「查看详情」，点击项目名 / 会话 ID 可直接查看对象。
- 详情支持近 7 / 30 / 90 天、近一年、全部本地记录和自定义日期；提供搜索、排序、分页及项目关联会话。
- 查看完整路径与时间、Token 拆分、模型构成、每日用量；项目补充 Git 提交、文本增删行和净增行。
- 详情按事件时间重算范围统计，跨日会话不会带入范围外用量；全部历史仅限本机保留记录，不展示对话正文。

### 修复内容

- 圆环、挂件与低额度提醒使用官方当前余量；接口失败时仅回退有效本地窗口。历史最高消耗不再覆盖当前值。
- 修复重置确认前、旧窗口迟到、连续重置及启动 / 失败回退缓存导致的余量错误；缺失数据保持未观测。
- 增加 GPT-6 Astra 标准 API 等价成本，修正缓存输入重复计费，并自动重算旧成本缓存。
...
- `0.5.2` 与 `0.6.0-dev.1` 均可升级到 `0.6.0`；保留本机设置，首次刷新自动迁移缓存，无需手工删除数据。
- 全部历史首次查询可能需要等待，后续在五分钟内复用采集结果；可点击「重新读取」刷新。
- API 等价成本是已知标准单价估算，不是实际账单；未知模型明确标记未定价，GPT-6 Codex credit 保持未定价。
- 安装包仍未签名。应用下载完成后由用户点击「重启并安装」，普通退出不安装。
...
- main CI run `33492001058` 与 Windows Package run `33492168620` 均成功；CI 已包含同期边界专项门禁。
- 正式安装包 `Codex.Companion.Setup.0.5.1.exe` 大小 `102964660` bytes，SHA256 为 `309D5C6A80A8CC858266DF58665B3E88DD9AE5B235256AF2BEBA676B54B40220`；实算结果与 GitHub asset digest、`SHA256SUMS.txt` 和匿名重新下载结果一致，产品名与产品版本为 `Codex Companion / 0.5.1`，Authenticode 为 `NotSigned`。
- blockmap 大小 `108430` bytes，SHA256 为 `0B7D97BFB2DCBB572D1BDEEF4A341F5F64A035E39F447EC4874E45ECF07024A4`；草稿 Release 初始缺少安装包和 `latest.yml`，发布流程从同一成功 Windows Package run 的 Actions artifact 取回，并先确认 artifact blockmap 与草稿 Release digest 一致，没有混用本机构建产物。
- `latest.yml` 大小 `359` bytes，SHA256 为 `1A3DD760EB212CC52B8007A34A6A896A87849C19D07FDB95DCD8072AE5ECBACB`，`version=0.5.1`，安装包路径、大小和 SHA512 完整；`SHA256SUMS.txt` 大小 `99` bytes，SHA256 为 `B1F21FE1D352084CC4F2C58D267F80189A0E0B81DD2E2528E83B90383A1BFE3C`。
