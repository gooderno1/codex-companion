  ...
  for (let attempt = 1; attempt <= MAX_CAPTURE_ATTEMPTS; attempt += 1) {
    ...
    const result = spawnSync(process.execPath, [electronCli, "."], {
      ...
      env: {
        ...
        CODEX_COMPANION_CAPTURE_PAGE: "repositories",
    ...
    console.warn(
      `第 ${attempt} 次生成 repositories ${viewport.width}x${viewport.height} 截图失败：${lastError}`
    );
  }

  throw new Error(`生成 repositories ${viewport.width}x${viewport.height} 截图失败：${lastError}`);
}
...
async function main() {
  ...
  for (const viewport of VIEWPORTS) {
    const outputPath = path.join(
      outputDir,
      `repositories-${viewport.width}x${viewport.height}-${suffix}.png`
    );
...
main().catch((error) => {
  console.error(`代码仓库页截图生成失败：${error instanceof Error ? error.message : String(error)}`);
  process.exitCode = 1;
});
