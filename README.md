...
- 当前开发版本：`v0.4.1-dev.2`；最新公开正式版为 `v0.4.0`，也是首个 updater-enabled 版本。
...
- 自动升级：`v0.4.1-dev.2` 已开启临时未签名升级模式；Windows NSIS 安装版可自动下载更新，但只有用户点击“重启并安装”后才执行，退出时不会静默安装。
...
- 已发布的 `v0.4.0` 把自动安装门禁编译为关闭状态，无法通过远端配置打开；升级到首个启用版本仍需手动安装一次。
