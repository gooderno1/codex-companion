...
- 当前版本：`v0.3.5-dev.1`
- 当前定位：开发迭代版，重点验证通知频率策略、通知页、设置页重排、Git 本机状态和 Windows 托盘品牌图标。
...
- 展示 Codex 赠送重置次数，支持查看每次重置的观测获取时间、预计过期时间和保守提醒时间。
- 后台刷新、启动刷新和手动刷新后按本机快照触发提醒：顶部铃铛保留快速摘要，通知页支持筛选、分页、详情和标记已读；默认“均衡”策略下同一提醒 `12h` 内不重复顶到列表或弹系统通知。
- 按模型聚合自然月 Token 与 API 等价成本。
- 按 Git 仓库归因 Codex 会话，并展示仓库提交、增删行与近期提交。
- 设置页支持 Codex 数据目录、仓库根目录、通知频率、计费月起始日、Git 本机身份状态、刷新历史摘要和本机数据边界查看。
...
- 本机通知状态：`notification-state.json`，保存提醒 key、标题、正文、类别、级别、触发时间、系统通知时间和已读时间；默认 `12h` 冷却避免 5 分钟自动刷新反复提醒，也用于通知页展示详情。
...
- 当前版本不请求 GitHub token；设置页的 Git 与授权区只展示本机 `git` 命令、`user.name` 和 `user.email` 状态。
