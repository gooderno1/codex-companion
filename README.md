...
- 当前开发版本：`v0.6.1-dev.1`；项目改为对应 Codex 保存的项目（Git 为可选关联），历史详情增加 SQLite 持久化增量索引和按需 Git 查询。首次建立索引需要时间，后续查询及重启复用索引。
- 当前正式版本：`v0.6.0`；项目概览与会话归因增加详情，支持近 7 / 30 / 90 天、近一年、全部本地记录和自定义日期，提供搜索、分页、关联会话、模型和每日用量。
- 本次还包含官方当前余量、迟到窗口、旧缓存迁移、GPT-6 Astra 成本与缓存输入去重计费修复。
