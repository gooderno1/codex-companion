...
- 当前版本：`v0.3.4`
- 当前定位：内部正式版，重点验证应用内提醒中心、通知详情和 Windows 托盘图标可见性。
...
- 后台刷新、启动刷新和手动刷新后按本机快照触发提醒：应用内提醒中心保留详情，系统通知作为外部提示；赠送重置临近保守使用时间、首次观测前已有重置无法反推过期时间、5H / 周额度剩余量低于 `20%` 或 `10%` 时提醒。
...
- 本机通知状态：`notification-state.json`，保存提醒 key、标题、正文、类别、级别、触发时间、系统通知时间和已读时间，避免 5 分钟自动刷新重复弹出同一提醒，也用于应用内提醒中心展示详情。
