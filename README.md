...
- 当前正式版本：`v0.5.2`；总览支持在同期百分比与绝对变化量之间切换，零分母按 `1` 计算具体百分比。
- 当前开发版本：`v0.6.0-dev.1`；项目概览与会话归因增加详情，支持近 7 / 30 / 90 天、近一年、全部本地记录和自定义日期，提供搜索、分页、关联会话、模型和每日用量。包含此前官方当前余量与 GPT-6 成本修复。
