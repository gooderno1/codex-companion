...
- 当前版本：`v0.3.6-dev.2`
- 当前定位：内部开发版，重点修正应用图标资产、赠送重置初始假定和观测延续口径、设置页单列布局和刷新历史独立页面。
...
- 展示 Codex 赠送重置次数，支持查看每次重置的观测获取时间、预计过期时间和保守提醒时间；当前口径默认排除已使用的 `2026-06-11` seed，首次未知 credit 按 `2026-06-14` 假定，`2026-06-30` 公开补偿和后续本地观测新增按 30 天有效期估算。
...
- 设置页按单列展示 Codex 数据目录、仓库根目录、通知频率、计费月起始日、Git 本机身份状态、刷新历史摘要和本机数据边界；完整刷新历史在独立页面分页查看。
