...
- 当前开发版本：`v0.6.2-dev.2`；会话列表与详情显示 Codex 自动名称，支持名称搜索；详情 Token 统一按总览自动换算为万 / 亿，悬停查看完整数值；详情和账本会话表支持点击表头升降序排列。
- 当前正式版本：`v0.6.1`；项目改为对应 Codex 保存的项目（Git 为可选关联），历史详情增加 SQLite 持久化增量索引和按需 Git 查询。首次建立索引需要时间，后续查询及重启复用索引。
