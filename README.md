...
- 当前版本：`v0.3.3-dev.1`
- 当前定位：开发版，重点增加本机系统通知，用于赠送重置到期提醒和 5H / 周额度低余量提醒。
...
- 后台刷新、启动刷新和手动刷新后按本机快照触发系统通知：赠送重置临近保守使用时间、首次观测前已有重置无法反推过期时间、5H / 周额度剩余量低于 `20%` 或 `10%`。
...
- 本机通知状态：`notification-state.json`，仅保存已发送提醒的去重 key、标题、正文和发送时间，避免 5 分钟自动刷新重复弹出同一提醒。
...
- 系统通知只使用聚合后的额度余量、赠送重置估算时间和本机快照状态，不包含账号、邮箱、原始 app-server 响应、用户输入正文或模型输出正文。
