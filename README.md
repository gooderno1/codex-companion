...
- 当前正式版本：v0.6.3，补齐 GPT-5.6 / Daybreak 标准价与 Astra credits，升级后自动重估旧成本。
- 当前开发版本：v0.7.0-dev.1，新增独立“额度估算”页，支持按月成本、周窗口等价值、价格快照切换和计算依据；后台原始索引支持跨重启复用。
- 定价快照核对日期为 2026-09-19；标准短上下文等价成本不是历史账单，不含未知缓存写入、长上下文和服务档位调整。
- 会话列表与详情显示 Codex 自动名称，支持名称搜索；详情 Token 按总览自动换算为万 / 亿，悬停查看完整数值；详情和账本会话表支持点击表头升降序排列。
- 项目对应 Codex 保存的项目，Git 为可选关联；历史详情使用 SQLite 持久化增量索引，首次建立后可跨重启复用。
- 历史详情支持：项目概览与会话归因增加详情，支持近 7 / 30 / 90 天、近一年、全部本地记录和自定义日期，提供搜索、分页、关联会话、模型和每日用量。
- 同时保留 v0.6.0 的官方当前余量、迟到窗口、旧缓存迁移、GPT-6 Astra 成本与缓存输入去重计费修复。
...
- 总览日、自然周、自然月、额度周和计费月的增减百分比使用上一周期相同时间进度；例如周三查看时只对比上周一到上周三同一时刻。
- 总览顶部可切换“百分比 / 数值”：百分比的上一周期同期为 `0` 时仅把计算分母替换为 `1`，超大结果紧凑显示为 `万% / 亿%`；数值模式显示真实 Token 或代码行数差值。选择保存在本机，切换不触发数据刷新。
...
- Windows 正式安装版可在设置页启用或关闭开机自启；默认关闭，配置保存在本机并同步到 Windows 登录启动项，源码开发模式不会注册 `electron.exe`。
