...
const rowName = (item: ActivityProject | ActivitySession) => item.name ?? ("sessionId" in item ? item.sessionId : "");
...
function TokenValue({ value }: { value: number }) {
  return <span title={exactTokenLabel(value)}>{formatCompactToken(value)}</span>;
}
...
function Breakdown({ item }: { item: ActivityTotals }) {
  return <>
    ...
    <dl className="activity-facts">
      {[["总 Token", item.tokens.total], ["输入总量", item.tokens.input], ["其中缓存输入", item.tokens.cachedInput], ["输出总量", item.tokens.output], ["其中推理输出", item.tokens.reasoningOutput], ["用量事件", item.events]].map(([label, value]) => <div key={label}><dt>{label}</dt><dd>{label === "用量事件" ? number(Number(value)) : <TokenValue value={Number(value)} />}</dd></div>)}
    </dl>
