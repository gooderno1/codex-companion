...
- 验证方式：Node `22.23.1` / npm `10.9.8` 下 `npm ci`、`npm run build`、`npm run verify:updater`、`npm run verify:quota`、`npm run verify:ledger` 和 `npm run verify:design` 通过；`npm run package:win`、`npm audit --audit-level=low` 和 `git diff --check` 通过；1360×900 开发态与 packaged 设置页人工截图通过；匿名访问仓库与 `v0.3.9` Release 资产返回 HTTP 200；packaged 构建真实请求更新 feed，在旧 Release 缺少 `latest.yml` 时按契约显示 `metadata-missing` 并降级到手动下载。
