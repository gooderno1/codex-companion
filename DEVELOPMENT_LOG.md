...
- 实现方式：新增一级“额度估算”，包含按月成本、模型 / 每日明细、周窗口图表及样本计算依据；支持近 6 / 12 个月、全部历史、自定义含首尾日期、价格快照选择、排序、分页和状态恢复。
- 统计定义：按本地自然月汇总；成本公式为 `(input-cached)×输入价 + cached×缓存价 + output×输出价`，单价单位为每百万 Token，reasoning 不重复加入 output。金额是本机已知部分的标准短上下文等价成本，不是订阅实付。
- 价格边界：复用 2026-09-17 / 2026-09-19 快照，未匹配模型保持未定价；未取得官方生效时间时“按当时价格”不返回历史金额，不将核查日当作生效日。Fast、工具、地区、缓存写入及完整上下文成本仍未知。
- 窗口规则：只匹配主池或早期未标识池的 10080 分钟周窗口；Spark 用量排除主池估值但保留在月度统计。5 分钟中位数降噪，重置 / 套餐 / 额度池变化、大于 3 点回落及超过 48h 中断分段。
- 样本条件：每段至少增长 10 个百分点，最多 48h；剔除过期、无效、99% / 100% 截顶及主池未定价 Token 超过 2% 的区间，不跨选择范围或核心已确认边界。
- 估算方式：`B=100×ΣΔC/ΣΔp`；至少 3 段、30 点且 `(P90-P10)/B≤0.5` 才标为中等证据，否则探索性或样本不足。P10–P90 是经验范围，另列 ±2min 对齐敏感性，图表缺口不插值。
- 变化提示：与一周内同套餐同池的前一中等证据窗口比较，最近 3 段的对齐范围均偏离原估值 20% 才提示变化；不能据此认定官方变更套餐。
- 数据实现：后台线程维护独立 SQLite 原始索引及时间索引；按文件 size / mtime 事务更新，重启复用、归档去重。价格更新不使原始索引失效；完整观测指纹缓存共享核心分析，切换价格只重估成本。
- 核心修复：固定远程 tag `codex-usage-core v0.2.0-dev.4`（46f0865），缓存回看时间解析。55,866 条观测的完整结果与 dev.3 相等，核心同机对比从 32.12s 降至 8.31s；dev-ledger v0.14.0-dev.29（8b21c83）同步升级、全流程验证并推送。
- 验证样例：合成 3 段共消耗 90 美元、增长 30 点，估值为 300 美元；100 万输入 / 80 万缓存 / 2 万输出，Astra 标准估值 3.80 美元；reasoning 不重复收费。
- 当前结果：功能完成；官方当前余量、通知和挂件口径不变。真实数据覆盖 550 个文件、9 个月，缓存命中查询 1.24s；重启复用 549 个文件，索引检查 219ms、首次派生查询 17.49s。首次建索引还需读取历史文件。
- 验证方式：npm run build；verify:estimation、verify:usage、verify:activity、verify:notifications、verify:updater；Electron 独立配置实测筛选、双向排序、详情、快照 / 历史口径、状态恢复及 1360×900 / 1080×720 布局；git diff --check。
- 交付边界：个人月度 / 周度结果与截图只保留 local_dev_work，不提交公开仓库；本次为开发版本，尚未正式 Release。
...
- 实现方式：创建本项目的价格维护专用任务，使用独立 worktree，仅传入必要维护规则；将现有 `automation` 的目标迁移到新任务，保持每 24 小时、ACTIVE 和原有计费核查 / 自动 Release 规则。
- 当前结果：只有一个调度；后续检查、发布结果及异常通知均进入专用任务。首次初始化只确认就绪，不提前核查或发布；更新规划文档中的执行位置。
- 验证方式：新任务初始化完成；回读配置逐字确认执行提示、目标任务、状态与间隔；`npm run build`、`git diff --check` 通过。本轮无应用实现或版本变更。

## [2026-09-18] v0.6.3-dev.2 docs(plan): 独立额度估算页与每日计费核查发布

- 开发原因：用户要求额度估算独立成页，并明确由 Codex 每 24 小时核查官网，发现计费更新后主动迭代和 Release。
- 实现方式：修订 `docs/quota-capacity-pricing-research-plan-2026-09-17.md`；一级导航“额度估算”与账本并列，规划月度成本 / 窗口估值、近 6 / 12 个月 / 全部 / 自定义范围、排序、状态保留及按需查询。
- 方案变更：价格随应用 Release 交付，取代独立远程目录下载；目录仍保留版本、来源和历史语义。沿用现有更新检查与安装偏好，升级后重算派生估值，官方余量口径不变。
- 自动化结果：已创建并启用当前任务内“核查模型价格并自动发布”，间隔 24 小时；检查官网与当前应用差异，首次运行也纳入应用已遗漏的模型价格。
- 触发规则：已确认且影响计价的新模型、费率、别名或计费规则触发必要实现、回归、构建、Git 同步与正式发布；仅排版、措辞或抓取时间变化不发布。未来费率不提前生效，未知字段不填 0 或借用相似模型。
- 发布规则：用户已授权例行自动迭代与 Release；核验准确提交 / tag 的 CI、Windows 打包、四项发布资产、版本、哈希与签名状态后公开为 stable。检查已有工作流和草稿以续接，保护未验收功能和无关工作区改动。
- 验证样例：已生效输入价由 4 变为 5 触发一次发布；网页换行不发版；来源冲突保留现值并通知；中断重试接续已有版本而非重复建 tag。
- 通知与隐私：无变化保持安静，成功发布或失败 / 需处理时通知；检查与去重状态仅存于忽略目录，不上传个人用量。本轮只修订页面方案并配置定时任务，未实现页面或立即发版，应用保持 v0.6.2。
- 验证方式：读取自动化配置确认 ACTIVE、每 24 小时和当前任务绑定；`npm run build` 与 `git diff --check` 通过。核心依赖保持 `@lifeinhand/codex-usage-core@0.2.0-dev.3`，本轮未修改核心逻辑。

## [2026-09-17] v0.6.3-dev.1 docs(research): 规划套餐额度估算与模型价格目录更新

- 开发原因：用户要求核实月 / 周限制与 Token 成本的关系，按历史数据估算每月已用成本，并研究周内变化和新模型价格自动更新。
- 研究范围：核对官方套餐、Codex Fast、API 模型价格和缓存计费资料；记录来源与核对日期。未找到固定月度美元预算或包含额度百分比到 credits 总量的完整公开公式，不将 API 标价当作内部成本。
- 实现方式：新增 `docs/quota-capacity-pricing-research-plan-2026-09-17.md`，区分官方余量、按月已记录标准成本、窗口满额等价值及可选月度参考值；保持应用版本 v0.6.2，不修改应用代码和统计契约。
- 计算规则：输入扣除已包含的缓存读 / 写 Token 后分项计价，推理不重复加入输出；历史价格和固定快照重估分开，未知模型显示未定价。合成样例：输入 100 万含缓存读取 80 万、输出 2 万，单价 10 / 1 / 50 美元每百万时为 $3.80。
- 估算规则：同池、同 duration、同 reset 片段使用 `100 × 成本增量 / 已用百分点增量`；实验采用 5 分钟桶、至少 10 个百分点、最长 48 小时、主池未知 Token 不超过 2%。排除跨 reset、迟到旧窗口和 99% / 100% 截顶；共享核心确认的边界不等于用户兑换重置次数。
- 当前结果：模型价格加权可作为额度变化的探索性解释，但不能证明官方采用固定预算；缺少档位、其他设备和完整账户活动时不推断官方调价或缩减。跨月窗口不整体划入首次活动月。
- 产品方案：先交付版本化价格目录与按月已记录成本，再提供分段窗口估值和证据详情；主余量与低额度提醒继续采用官方当前值。价格更新与应用版本解耦，原始计数和派生估值分离，避免每次重估重新解析全部会话。
- 自动更新方案：官方来源差异检测、结构化目录校验、版本发布、客户端更新与离线回退；分别建模 API / credits、档位、缓存、长上下文与生效时间。初期新模型需维护者核验，未知计费维度不自动套用相近模型价格；本轮未创建定时任务。
- 已知问题：现有硬编码目录缺少部分新模型费率，导致已记录成本低估；本轮仅记录，待实施价格目录时修复。历史 Fast 档位和真实套餐变更时间仍不完整。
- 隐私边界：个人月度 / 周度结果、数据副本与复现脚本仅保存在忽略目录 `local_dev_work/quota-research-2026-09-17/`，公开仓库仅提交通用方法和合成样例。
- 验证方式：复用采集器核查计数关系，按月 / 模型交叉汇总；比较 5 / 10 / 15 点阈值、1 / 5 分钟桶、时间对齐偏移及相邻区间预测。`npm run build`（lint、双端类型检查、前后端构建）与 `git diff --check` 通过；核心远端最新 tag 与依赖均为 `@lifeinhand/codex-usage-core@0.2.0-dev.3`。
...
- 设计依据：先编写 `docs/activity-sorting-design-2026-09-17.md`，沿用总览表头的箭头、高亮和新字段首次升序规则。
- 实现方式：增加 `TableSortHeader` 和共享比较器；详情主列表、模型构成、每日用量、账本会话表支持双向排序，表头提供 aria-sort 与下一步操作标签。
- 交互规则：详情主列表默认最近活动降序，模型默认 Token 降序，每日默认日期降序。主列表下拉框与方向按钮同步；项目 / 会话分别保留排序状态，切换对象重置子表排序。
- 排序范围：先对全部筛选结果排序，再每页取 25 条；账本对当前快照全部已加载会话排序，再展示前 8 条。排序回到第一页并保留时间范围、搜索、项目筛选和已选对象。
- 比较规则：使用原始数值 / 时间戳，名称使用中文自然排序；同值按 ID / key 稳定排序，零为有效数值。缺失时间和模型未定价成本始终置后；部分定价合计继续按已知金额排序并保留提示。
- 验证样例：60 条降序输入，升序第一页得到全局最小的 25 条，账本首项来自原第 60 条；万 / 亿显示不参与比较；同值行在切换方向后仍保持稳定键顺序。
- 当前结果：项目、会话和对象子表可直接点击表头双向排列；排序仅发生在渲染端，无新增 IPC、数据库、会话解析或 Git 请求，统计口径不变。
- 验证方式：`npm run build`、`npm run verify:activity`（新增跨页排序、缺失值、稳定顺序、原数组不变检查）、`npm run verify:ledger`、`git diff --check` 通过。
- 验证方式：隔离 Electron 使用真实数据响应固定测试期间数据，验证 31 个项目、494 个会话的跨页 Token 双向排序、搜索与项目筛选保留、选中项保留、汇总不变、子表排序和账本全量排序后取 8 条。所有操作仅首次加载触发 1 次详情 IPC。
- 验证方式：检查 1360 / 1080 窗口截图，表头箭头、工具栏和窄窗口内容正常显示。核心包保持 `@lifeinhand/codex-usage-core@0.2.0-dev.3`。

## [2026-09-17] v0.6.2-dev.1 feat(activity): 显示会话名称并统一 Token 自动单位

- 开发原因：会话只显示 ID，不易识别；历史详情的 Token 长整数与总览的万 / 亿单位不一致。
- 设计依据：先编写 `docs/session-name-token-display-2026-09-17.md`，核对真实名称来源及总览现有单位规则。
- 来源核查：本机 148 条 `threads.name` 与 `session_index.jsonl` 的 `thread_name` 一致；525 条数据库记录中 472 条 `title` 等于首条输入，因此不把 title 当作会话名称。
- 实现方式：新增只读名称采集，优先使用非空 `threads.name`，其次取名称索引中同 ID 最新有效 `thread_name`。索引按 `updated_at` 排序，同时间取靠后行；损坏单行、旧 schema 或数据库暂不可读时仍可回退。
- 数据边界：只读取名称、ID 和名称索引时间，不读取 title 或任务正文；主快照可保存名称，历史 Token SQLite 不新增名称字段。名称在查询 / 刷新时更新，改名不触发 JSONL 重解析。
- 实现方式：账本列表、单次峰值说明、详情列表和对象标题优先显示名称；ID 保留作稳定选择键、辅助信息和详情完整字段。缺名回退 ID，详情支持名称 / ID 搜索与名称排序。
- 实现方式：总览格式化函数移到 `src/shared/tokenFormat.ts`，详情汇总、列表、分项、模型及日用量共用。小于 10,000 显示整数，10,000 起显示万，100,000,000 起显示亿。
- 精度规则：万值原量小于 100,000 时保留 1 位小数，否则 0 位；亿值原量小于 1,000,000,000 时保留 1 位，否则 0 位。会话数、事件数、Git 行数和美元金额不使用 Token 换算。
- 验证样例：12,500 显示 1.3 万；123,456,789 显示 1.2 亿，悬停显示 123,456,789 Token。修改会话名称后再次查询得到新名称，解析文件数保持 0。
- 当前结果：会话名称与 ID 可同时辨识；所有详情层级使用一致的 Token 单位，完整原值仍可查看。同步数据契约、隐私说明、组件映射和页面文档。
- 验证方式：完整构建、`verify:activity`（含名称来源 / 回退 / 改名及单位边界）、`verify:ledger`、`git diff --check`；新增专项随现有详情命令进入 CI。
- 验证方式：真实 Electron 检查名称搜索、对象标题、完整 ID、无名回退、账本辅助 ID、10 处 Token 字段格式与完整数值提示、事件整数和 1360 / 1080 窗口布局；检查并修正汇总数值继承标签字号的问题。

## [2026-09-17] v0.6.1 release: 发布 Codex 项目归因与持久化查询索引

- 开发原因：用户要求将已验证的修复发布为正式 Release，供升级测试。
- 发布范围：包含 `v0.6.1-dev.1`，无排除版本；核心依赖继续固定 `@lifeinhand/codex-usage-core@0.2.0-dev.3`。
- 实现方式：应用和 clientVersion 统一为 `0.6.1`，同步 README、数据契约、组件映射与 Release notes；通过正式 tag 工作流生成同源 Windows 安装包和更新资产。
- 当前结果：发布 Codex 项目身份统一、非 Git 项目支持、独立 SQLite 持久化增量索引、按需 Git 查询和旧缓存迁移。
- 验证方式：正式版完整构建、详情、用量、更新器和签名政策专项通过；正式提交 `a55846d` 的 CI run `35219473329` 与 Windows Package run `35219477013` 均成功。
- 资产验证：四项产物均来自同一次 tag 构建；自动上传产生两份同标签草稿，补齐完整资产后清理本次重复草稿。安装包 `102984092` bytes，SHA256 `872F0C6B695703D1C8883741F557DC36CE99E4A864039867B1081687E50F3EDF`，产品名 / 版本为 `Codex Companion / 0.6.1`，Authenticode 为 `NotSigned`。
- 发布结果：`v0.6.1` 已公开为 stable Latest；四项资产匿名下载哈希与工作流产物一致，安装包 SHA512 / 大小与 latest.yml 匹配，稳定更新入口和公开 Release 页面通过验证。
- 升级注意：首次建立历史索引仍需等待，本机 526 个文件约 25.2 秒；完成后全历史查询约 0.82 秒，重启后约 0.84 秒。安装包保持既有未签名发布方式。

## [2026-09-17] v0.6.1-dev.1 fix(activity): 按 Codex 项目归因并增加持久化查询索引

- 开发原因：v0.6.0 错把 Git 仓库作为项目主表，无 Git 的 Codex 项目无法正确显示；详情只有进程内缓存且阻塞等待所有仓库历史，本机首次查询耗时 31.134 秒。
- 设计依据：先编写 `docs/codex-project-index-plan-2026-09-17.md`；用户已明确 Codex 项目为主、Git 只是可选关联。
- 实现方式：只读 Codex `projects / project_roots / threads.project_id` 和桌面项目字段，合并新旧 ID；以数据库显式归属、桌面归属、明确无项目、旧版 root hint、cwd 最长根路径依次匹配。
- 归因边界：无 Git 项目照常显示；多个 Codex 项目关联同一仓库不合并。相同根目录多个候选、显式已删除项目或无法匹配时保留「无项目 / 未匹配」，不按目录名称猜测。
- 实现方式：总览、会话归因和详情共用项目映射；名称与归属变化无需重解析 JSONL。代码行数和提交不可用时显示「—」，原仓库页保持 Git 视图。
- 缓存迁移：新快照 `projectAttributionVersion=1`；旧项目分组缓存不可返回为当前项目，采集失败回退时也清除旧分组。
- 实现方式：增加 Companion 自有 SQLite 索引，保存文件 size / mtime、结构化会话统计及事件。使用 `events_at` 时间索引和左闭右开查询、WAL 与事务替换；新增 / 变化文件才解析，归档移动 / 截断 / 删除会移除旧事件。
- 索引边界：读取期间变化的文件标记待重读；同目录并发刷新合并；不同 Codex home 独立数据库。索引版本关联 `CODEX_SESSION_CACHE_VERSION`，定价变化可重建；首次可复用版本和文件签名一致的普通缓存。
- 实现方式：详情不再等待 Git 全量采集；选中项目后点击「读取 Git 活动」单独加载，按项目 / 根目录 / 范围缓存 60 秒，重新读取强制跳过缓存。无仓库与读取失败分别显示，不把仓库活动归给单个会话。
- 核心依赖：已核对远程最新 tag，继续固定 `@lifeinhand/codex-usage-core@0.2.0-dev.3`；仅导出现有解析函数和缓存版本供本地索引复用，未修改 Token / reset 算法。
- 验证样例：同一文件先有 100 Token，追加累计 180 后索引合计为 180；右边界事件排除。移动到归档不重复计数，截断及删除移除旧事件。SQL 查询计划命中 `events_at`。
- 验证样例：有 / 无 Git、嵌套根、显式工作树、项目改名 / 重新归属、新旧 ID 迁移、旧版工作目录、同目录歧义、已删除项目和只读数据库边界均有回归。
- 当前结果：本机 526 个文件、493 条有用量会话，识别 30 个 Codex 项目及一个无项目分组。首次索引 25.185 秒；近 7 天 0.216 秒，全历史 0.823 秒，关闭数据库并重建服务后 0.838 秒且复用全部文件。
- 性能边界：首次建立全历史索引仍需等待；常规仪表盘 Git 刷新保留既有路径。实际时间随历史大小和磁盘性能变化。
- 验证方式：完整构建（lint / TypeScript / renderer / main）、`verify:activity`、`verify:usage`、`verify:comparisons`、`verify:ledger`、`git diff --check`；新增项目索引验证随 `verify:activity` 纳入 CI。
- 验证方式：隔离真实 Electron 实例检查项目 / 会话双入口、按需 Git、非 Git 空态、全历史、日期校验、分页、搜索、1080 / 1360 窗口、关闭和焦点恢复。

## [2026-09-17] v0.6.0 release: 发布项目与会话详情及用量修复

- 开发原因：用户要求发布正式 Release，升级后自行测试。
- 发布范围：包含 `v0.5.3-dev.1`、`v0.5.3-dev.2`、`v0.6.0-dev.1`，无排除版本；`0.5.3` 的修复随 `0.6.0` 一起正式发布。
- 实现方式：应用 / clientVersion 统一为 `0.6.0`，更新 README、数据契约、组件映射及 Release notes；核心包继续固定远程 tag `v0.2.0-dev.3`。
- 当前结果：保留已验收的项目与会话历史详情、官方当前余量、窗口与缓存修复、GPT-6 成本和缓存去重计费，正式产物由 tag 工作流构建。
- 验证方式：正式版完整构建与详情、用量、同期、账本、通知、更新器、开机自启、签名政策专项全部通过；发布提交 `4df3796` 的 CI run `35205778399` 和 Windows Package run `35205782744` 均成功。
- 资产验证：使用同一次 tag 构建的安装包、blockmap、latest.yml 与 SHA256 清单；补齐草稿缺少的 blockmap，没有混用本机构建产物。
- 资产验证：安装包大小 `102977525` bytes，产品名 / 版本为 `Codex Companion / 0.6.0`，Authenticode 为 `NotSigned`；SHA256 `5E7B4F4D6106381AB7425A9EBA07D3A1B378F7BC07E5201850719587C916EFD1`，与 GitHub digest、清单及匿名重新下载结果一致。
- 发布结果：`v0.6.0` 已发布为 stable Latest；Release 页面、blockmap、清单及稳定更新入口匿名 HTTP `200`，公开 latest.yml 与构建产物相同并指向 `0.6.0`。本机保留 `0.6.0-dev.1`，供用户自行测试升级。

## [2026-09-17] v0.6.0-dev.1 feat(activity): 项目概览与会话归因增加历史详情

- 开发原因：项目概览仅有当前周期，会话归因首屏只显示 8 条；默认会话文件扫描为 60 天，无法查看更长历史与完整归因。
- 设计依据：先编写 `docs/activity-details-design-2026-09-17.md`，定义入口、布局、时间边界、字段、加载与空态、隐私及验证路径，再修改实现。
- 实现方式：两个模块增加「查看详情」，项目名 / 会话 ID 支持对象直达。原页面筛选保持，详情使用独立大对话框，可关闭、Escape 退出并恢复焦点。
- 实现方式：提供入口周期、近 7 / 30 / 90 天、近一年、全部本地记录及自定义日期。快捷范围按本地自然日包含今天；自定义结束日包含全天，实际事件查询为左闭右开，未来部分截到当前时间。
- 实现方式：新增 `activity:details` IPC、独立 `ActivityDetailsService`、详情契约与渲染组件；按需扫描全部本地 sessions / archived_sessions，主快照维持 60 天范围。
- 缓存边界：详情初次读取已有解析缓存，仅在独立内存维护新缓存，不写回常规磁盘缓存；同配置并发采集合并，采集结果复用 5 分钟，重新读取会检查变化。
- 实现方式：按事件时间重新汇总项目、会话、模型和日用量；跨日会话只计入范围内事件。输入包含缓存、输出包含推理，分项不重复相加。
- 实现方式：项目沿用已有 cwd 到 Git 根目录归因；未归因会话单独保留，项目 / 会话总 Token 守恒。项目可跳转关联会话，支持搜索、排序与每页 25 项分页。
- 实现方式：对象详情显示完整 ID / 路径 / 时间、Token 拆分、模型成本与每日用量；项目另列 HEAD 可达提交的文本增删行，读取失败明确为未知，不把仓库活动硬归到单个会话。
- 数据边界：全部仅指本机保留记录，展示首末可观测时间和扫描文件数；未知模型标为未定价，合计为已知单价部分；不展示对话、工具命令或凭据。
- 核心依赖：开发前核对远端最新 tag 为 `@lifeinhand/codex-usage-core@0.2.0-dev.3`，继续固定该版本；仅增加本应用的采集范围参数与详情映射，不修改解析、Token 增量或 reset 引擎。
- 验证样例：跨日会话范围内两条事件共 `320 Token`，范围外事件排除；右边界事件不计入；未归因另有 `60 Token` 时项目 / 会话合计同为 `380`。旧归档文件默认扫描为 `0` 条，详情扫描可读 `1` 条并复用缓存。
- 验证样例：临时 Git 仓库包含范围前、范围内、右边界各一次提交，详情仅计入 `1` 次提交 / 新增 `1` 行。补测修正了 Git format 参数，失败分支仍保持未知。
- 当前结果：真实历史查询读取 `525` 个文件、`492` 条有用量记录的会话、`18` 个项目分组，覆盖最早 `2026-01-20T06:35:28.585Z`，首次查询约 `20s`；不再局限于最近 60 天。
- 验证方式：完整构建（lint / TypeScript / renderer / main）、`verify:activity`、`verify:usage`、`verify:ledger`、`verify:comparisons` 与空白检查；新增详情回归加入 CI。
- 验证方式：真实 Electron 验收双入口、对象直达、全历史分页、搜索空态、非法日期拒绝、自定义 2026 年 1 月历史、项目到关联会话、关闭与焦点恢复；检查 `1360×900`、`1080×720` 截图。隔离验收资料位于不提交的 `local_dev_work/`。
- 打包验证：完整构建、详情专项与签名政策检查通过；Windows 安装包 `Codex.Companion.Setup.0.6.0-dev.1.exe` 为 `103325106` bytes，SHA256 `DB0B0D5FECBEFD0FD338C588D36B2F597C8450BD8D2129994F8B3C207AA598AE`。
- 安装验证：本机从 `0.5.3-dev.2` 原位升级到 `0.6.0-dev.1`，安装器退出码 `0`；已核对主程序产品版本和安装包内详情 IPC / 全历史查询实现，并重新启动应用。

## [2026-09-17] v0.5.3-dev.2 fix(quota): 根治当前余量被历史窗口和旧缓存覆盖

- 开发原因：`dev.1` 只在历史 reset 已确认后归回迟到观测，确认前仍可能显示 `2%`；圆环和通知还把历史最高消耗作为当前额度。
- 口径确认：用户确认圆环、挂件和低额度提醒采用官方当前余量；接口失败时使用本地最新有效窗口。历史累计用于账本和价值估算，不再覆盖当前值。
- 实现方式：先检查核心远程更新，再在共享核心修正窗口校验、观测归属、reset 确认和去重；本项目固定远程 tag `@lifeinhand/codex-usage-core@0.2.0-dev.3`（`d6affd5`）。另一下游同步为 dev-ledger `0.14.0-dev.28`。
- 实现方式：窗口身份使用 `resetsAt + windowMinutes`；本地候选按额度池与窗口类型隔离，选择最新有效截止时间内的最新观测，允许 `60s` 边界容差，不依赖消耗下降阈值。
- 排除条件：过期、非法百分比、非法时间、非正时长、超前超过 `5min` 的观测或窗口起点均不能作为当前值。官方成功响应的缺失窗口保持未观测，不补历史值。
- 实现方式：当前余量恒为 `100 - 当前 usedPercent`，截止时间也来自当前窗口；当前周历史证据只接受本窗口身份，Token 仍按真实发生时间归属。
- 实现方式：旧窗口迟到不再否定稳定的新窗口；`30min / 6h` 确认条件保留，后续独立 reset 截止前次确认区间，取消跨窗口的 `12h` 相似度去重。
- 实现方式：快照增加 `quotaDisplayVersion=2`；启动与进程内缓存需语义版本匹配、年龄不超过 `60s` 且当前窗口未到期。采集异常清空当前额度并保留历史时间，不伪装成新快照。
- 验证样例：确认前预期 `99%` 的样例从 `2%` 修正为 `99%`；迟到旧窗口导致的 reset `0 -> 1`、`2h` 内连续 reset `1 -> 2` 均通过；新窗口已用 `99%`、迟到旧窗口 `98%` 时仍选新窗口。
- 当前结果：本机隔离采集在 `2026-09-17T05:22Z` 与官方同为已用 `54% / 剩余46%`，截止 `2026-09-23T08:07:10Z`。真实 Electron 页面确认来源文案、当前余量、GPT-6 和非零成本显示正常。
- 验证方式：核心 `npm test`；Companion `npm run build`、`npm run verify:usage`、额度快照、同期、通知、账本专项与 `git diff --check`。覆盖确认前、迟到、乱序、连续 reset、过期、缺失、池隔离、通知、挂件和缓存迁移。
...
- 开发原因：已安装的 `v0.5.2` 在用户重置后仍显示周余量 `2%`，GPT-6 Astra 会话的 API 等价成本为 `0`。
- 开发原因：新周期实际起点为 `2026-09-16T08:07:10Z`；`08:07:20.955Z` 写入的旧窗口 `98%` 观测被按记录时间放进新周期，覆盖了新窗口 `51%` 的高水位。
- 实现方式：先检查核心包远程 HEAD 和全部 tag；最新仍为 `@lifeinhand/codex-usage-core@0.2.0-dev.1`（`644691e`），本次不升级依赖。
- 实现方式：共享核心包已经正确确认 reset；仅修正 Companion 的周期映射。已确认 reset 后，旧截止时间匹配在 `5min` 容差内且更接近旧边界的延迟观测归回旧周期；相同窗口、未确认 reset、5H 和 Token 时间归属维持原规则。
- 实现方式：新旧截止时间相距小于 `5min` 时按距离排除新窗口，距离相同不重分配。原始观测时间和旧周期 reset 证据均保留。
- 实现方式：按 `2026-09-17` 官方 GPT-6 Astra 模型页补充每百万非缓存输入 / 缓存输入 / 输出 `$10 / $1 / $50`；仅接受已知名称及日期快照，未知型号不套用相邻模型单价。
- 实现方式：输入总量已包含缓存，美元及已知 credit 公式统一使用 `max(0, input - cachedInput)` 计算非缓存部分。`100万` 输入含 `80万` 缓存、`10万` 输出的 GPT-6 标准估算为 `$7.80`。
- 实现方式：会话缓存版本从 `1` 升至 `2`，升级后重算未修改文件的派生成本，后续恢复增量复用。新增 `verify:usage` 并加入 CI。
- 当前结果：本机隔离采集快照从周余量 `2% / 成本 $0` 修正为 `49% / $172.577424`（`2026-09-17T04:50:53.864Z`）；额度快照校验通过，周期起止和 Token 归属正确。
- 验证方式：完整构建（lint、TypeScript、renderer 和 main）；`verify:usage` 覆盖双 session 延迟样本、相距 `2min` 的新旧边界、旧周期证据、额度池隔离、同窗口高水位、5H 缺失、模型价格、缓存去重和旧缓存迁移。
- 验证方式：同期、通知、账本和更新器专项校验通过；真实 Electron 总览截图确认周余量 `49%`、GPT-6 模型行和非零成本显示正常；隔离配置与证据保存在 Git 忽略的 `local_dev_work/usage-fix-review/`。
- 验证方式：最终源码再次通过完整构建与用量回归；Windows NSIS 安装包为 `103192265` bytes，产品版本 `0.5.3-dev.1`，SHA256 `3AEDF50341805273B02DF06BD3B52D64BE20F6736D73854B35635653FBD162F2`；签名政策校验和 `git diff --check` 通过。
- 遗留问题：成本为标准单价估算，未还原长上下文、缓存写入和服务档位的请求级调整；GPT-6 Codex credit 未核实，保持未定价。开发安装包需安装后才能替换正在运行的 `v0.5.2`。
...
- 实现方式：应用版本与官方只读请求的 `clientVersion` 收敛为 `0.5.2`；README、Roadmap、自动升级规划、数据契约、组件映射和 Release notes 同步正式口径；共享核心包继续固定为 `@lifeinhand/codex-usage-core@0.2.0-dev.1`，不修改 session 解析、额度 reset 引擎或当前值统计。
- 当前结果：正式版保留百分比默认值，并允许用户在顶部切换为真实绝对差值；旧配置自动迁移，选择仅保存在本机。开发提交 `cb48e24` 和 CI run `33498506648` 已通过；本地已生成 `0.5.2` Windows NSIS 安装包。
- 验证方式：正式版完整构建、同期/额度/账本/更新器/通知/开机自启/设计/签名政策专项、依赖审计和空白检查全部通过；本地安装包大小 `103192340` bytes，SHA256 `BFFBA322C0EE552F328CDE6D16396DB9EA893795D74CC1FD5B6F36B2803C7E83`，Authenticode 为 `NotSigned`；blockmap `108681` bytes，`latest.yml` `359` bytes且 SHA512 与安装包实算一致，主程序与安装包产品名 / 版本均为 `Codex Companion / 0.5.2`。
...
- 实现方式：新增正式规划文档，定义零分母公式、超大百分比紧凑格式、绝对差值单位、顶部工具栏切换位置、本机持久化方案、数据不变项、影响文件和实施顺序。
- 当前结果：推荐只替换百分比公式分母，百分比模式显示 `万% / 亿%` 紧凑值并保留完整提示；绝对值模式显示带符号的 Token 或代码行数差值；切换与“时间视角”并列，默认百分比并跨重启记忆。应用源码、package 版本和 Release 均未修改。
- 验证方式：核对 `v0.5.1` live 快照，确认本月 Token `125,778,327 / 上月同期 0`、今日代码 `6,541 / 昨日同期 0`；检查现有顶部工具栏、四卡、`AppPreferences` 和响应式布局结构；执行 `git diff --check` 验证规划文档空白格式。
...
- 实现方式：应用版本与两个官方只读请求的 `clientVersion` 从 `0.5.1-dev.1` 收敛为 `0.5.1`；README、Roadmap、自动升级规划、数据契约、组件映射和 Release notes 同步正式版口径；核心包继续固定为 `@lifeinhand/codex-usage-core@0.2.0-dev.1`，不修改 session 解析、额度 reset 引擎或当前周期统计。
- 当前结果：正式版资料已收敛；顶部变化百分比按上一周期同期计算，短月按上一周期末封顶，当前周期绝对 Token、额度余量和 reset 证据不变；本地安装包大小 `103165583` bytes，SHA256 `4736D7FCE4A218E5BF13AC73C9C446803E1FF3A04AE8BE5C143103264C774935`，Authenticode 为 `NotSigned`；应用主程序与安装包产品版本均为 `0.5.1`。正式 tag 继续由 GitHub Actions 生成唯一 Release 资产。
- 验证方式：开发提交 CI run `33491602236` 已通过全量构建、更新器、通知、同期边界、开机自启、签名政策和空白门禁；正式版本执行全量构建、同期/额度/账本/更新器/通知/开机自启/设计 token/签名政策专项、零漏洞审计、Windows NSIS 打包与 `git diff --check`；正式 CI、Windows Package、Release 资产和匿名下载结果在发布后记录。
...
- 实现方式：记录 main CI run `33387216757`、Windows Package run `33387341753` 和四项正式资产；草稿 Release 初始只有 blockmap 与 SHA256 清单，因此从同一成功 workflow 的 Actions artifact 取回安装包和 `latest.yml`，先确认 artifact blockmap 与草稿 Release digest 一致，再补齐缺失资产并发布为 Latest。
- 当前结果：`v0.5.0` 已发布为 latest Release；正式安装包大小 `102964293` bytes，SHA256 `794D1A47651937526D056EA9DEE68A1AB81F27018711C63AC21E9EC78E43CCFB`；blockmap 大小 `108549` bytes；`latest.yml` 大小 `359` bytes，版本、路径、大小和 SHA512 与安装包一致；`SHA256SUMS.txt` 大小 `99` bytes。
- 验证方式：两个正式 GitHub Actions workflow 均成功；安装包实算 SHA256 与 GitHub asset digest、`SHA256SUMS.txt`、匿名重新下载结果四方一致；Release 页面、安装包、blockmap、tag `latest.yml`、`SHA256SUMS.txt` 和 `/releases/latest/download/latest.yml` 匿名 HTTP 访问均返回 `200`。
...
- 实现方式：应用版本与 app-server `clientVersion` 从 `0.4.5-dev.1` 收敛为 `0.4.5`；README、Roadmap、数据合同、组件映射、自动升级规划和 Release notes 同步正式版口径；正式 tag 继续由 GitHub Actions 生成唯一 Release 资产。
- 当前结果：正式 Windows NSIS 安装版在自动检查开启时按启动约 `15s` 和每 `6h` 调度检查；符合门禁的新版本会自动下载，已发现版本后再打开自动下载也立即开始；更新界面保留状态和操作，移除重复说明小字。
- 验证方式：正式收敛前已通过全量构建、更新器专项、设计 token、签名政策、零漏洞审计、Windows NSIS 打包与 `1360×900` 设置页截图检查；正式版本再次执行相同验证。本地正式安装包大小 `102759963` bytes，SHA256 `5C8FEDAB41481C55F52CA9971D8B49922F7D1FD6CEE466B0C0092A03E132CA16`，Authenticode 为 `NotSigned`，同时生成 `108383` bytes blockmap 和版本、路径、大小、SHA512 完整的 `359` bytes `latest.yml`。
- 遗留问题：安装包仍未签名，Windows SmartScreen 或企业策略可能拦截；普通退出不会安装，仍需用户点击“重启并安装”。
...
- 实现方式：记录 CI run `30353771174`、Windows Package run `30353774344`、正式安装包与更新元数据；electron-builder 的 draft Release 只出现 blockmap，安装包与 `latest.yml` 仅存在于同一成功 workflow 的 Actions artifact，因此先用 artifact blockmap SHA256 与 Release blockmap digest 确认同源，再核对安装包与 `SHA256SUMS.txt` 后补齐缺失资产。
- 当前结果：`v0.4.4` 已发布为 latest Release；正式安装包大小 `102651500` bytes，SHA256 `576A697EBCEF3DE86842BE649D383FCB2781AAE0449B4662EDF186A515921D9A`；blockmap 大小 `107918` bytes；`latest.yml` 大小 `359` bytes，版本、路径、大小和 SHA512 与安装包一致；`SHA256SUMS.txt` 大小 `99` bytes。
- 验证方式：两个正式 GitHub Actions workflow 均成功；安装包实算 SHA256 与 GitHub asset digest、`SHA256SUMS.txt` 三方一致；Release 页面、安装包、blockmap、tag `latest.yml`、`SHA256SUMS.txt` 和 `/releases/latest/download/latest.yml` 匿名 HTTP 访问均返回 `200`。

## [2026-07-28] v0.4.4 release: 发布升级与通知交互修复

- 开发原因：`v0.4.4-dev.1` 已修复真实升级时前台命令行窗口持续显示和单条通知无法直达对应详情的问题，需要尽快发布补丁版。
- 实现方式：应用版本与 app-server `clientVersion` 从 `0.4.4-dev.1` 收敛为 `0.4.4`；README、Roadmap、数据合同、组件映射、自动升级规划和 Release notes 同步正式版口径；正式 tag 继续由 GitHub Actions 构建唯一 Release 资产。
- 当前结果：`v0.4.4` 起后续自动升级使用无控制台 VBScript 启动器；通知深链接保留目标 key 并无重载定位对应详情；本地正式安装包大小 `102745114` bytes，SHA256 `05E4F31CDC6901606DCBD6873C42C02073C7B15A19D7D405F245EC9365FFFBE2`，Authenticode 为 `NotSigned`。
- 验证方式：正式收敛前已通过全量构建、隐藏 helper 实际进程 smoke、通知跳转专项和真实历史截图、签名政策、零漏洞审计、Windows NSIS 打包与空白检查；`v0.4.4` 再次执行相同验证通过，生成 `108401` bytes blockmap 和版本、路径、大小、SHA512 完整的 `359` bytes `latest.yml`；开发提交 CI run `30353622218` 通过。
- 遗留问题：从 `v0.4.3` 安装本版本时仍由旧版代码发起 helper，因此该次升级仍可能显示一次 PowerShell 窗口；升级到 `v0.4.4` 后才对后续版本生效。
...
- 实现方式：使用与 CI 一致的 npm `10.9.8` 重新生成 lockfile，显式保留两个 `@emnapi` 可选 peer 及其 `wasi-threads` 依赖，并把 npm 自动规范化为 SSH 的共享核心包 resolved URL 恢复为公开 HTTPS；安装包路径断言改为比较双方 `realpath`，继续验证真实路径 confinement，不依赖短路径表示。
...
- 当前结果：本地生成 `Codex.Companion.Setup.0.4.1-dev.1.exe`；应用主程序与安装包的 `ProductName` 均为 `Codex Companion`，`ProductVersion / FileVersion` 均为 `0.4.1-dev.1`。主程序 SHA256 为 `E0AF8EA3FAA4E812E79C17C218F4FCF0550239D438AED7A23C61F4AA57C3D840`，安装包 SHA256 为 `EE93A4ADF5D1593995AE4AE1DE4359CCBFC4CECE14A1AE8A0BF0EC95812029EA`；两者 Authenticode 仍为 `NotSigned`，不会提前打开自动安装。现有 `v0.4.0` GitHub Release 正文已补 `Code signing policy` 链接并明确该资产仍未签名。
- 验证方式：Node `22.23.1` / npm `10.9.8` 下执行 `npm ci`、`npm run package:win`、`npm run verify:signing-policy`、`npm run verify:updater`、`npm audit --audit-level=low` 和 `git diff --check`；打包、政策合同、更新器合同与空白检查通过，依赖审计为 `0` 个漏洞；使用 PowerShell `VersionInfo` 和 `Get-AuthenticodeSignature` 逐项核对应用主程序与安装包。GitHub CI run `29417686697` 通过；手动 Windows Package run `29417705637` 在 GitHub 托管 runner 上完成打包、产物元数据检查和 Actions artifact 上传。
...
- 当前结果：GitHub Actions 正式生成 `Codex.Companion.Setup.0.4.0.exe`，大小 `105455204` bytes，SHA256 为 `E6881551C43BBC9EBAC86BED22474C5E93BE5A24D6962DC1BD1D85825CCF4C86`；`latest.yml` 固定 `version=0.4.0`、安装包路径、大小和 SHA512，Release 已公开并标记为 latest。
- 验证方式：Node `22.23.1` / npm `10.9.8` 下执行 `npm ci`、`npm run package:win`、`npm run verify:updater`、`npm run verify:quota`、`npm run verify:ledger`、`npm run verify:design`、`npm audit --audit-level=low` 和 `git diff --check`；依赖审计为 `0` 个漏洞；main CI 与 tag 发布 workflow 通过；重新下载远端正式安装包后，计算结果与 `SHA256SUMS.txt` 一致，四个 Release 资产匿名 HTTP 访问均返回 `200`。
...
- 验证方式：Node `22.23.1` / npm `10.9.8` 下 `npm ci`、`npm run build`、`npm run verify:updater`、`npm run verify:quota`、`npm run verify:ledger` 和 `npm run verify:design` 通过；禁用本机 Git credential helper 后，仍可通过公开 HTTPS tag 完成 `npm ci`；`npm run package:win`、`npm audit --audit-level=low` 和 `git diff --check` 通过；1360×900 开发态与 packaged 设置页人工截图通过；匿名访问两个仓库与 `v0.3.9` Release 资产返回 HTTP 200；packaged 构建真实请求更新 feed，在旧 Release 缺少 `latest.yml` 时按契约显示 `metadata-missing` 并降级到手动下载。
